[![view on npm](http://img.shields.io/npm/v/@pixida/pixida-aws-azure-login.svg)](https://www.npmjs.org/package/@pixida/pixida-aws-azure-login)
[![npm module downloads per month](http://img.shields.io/npm/dm/@pixida/pixida-aws-azure-login.svg)](https://www.npmjs.org/package/@pixida/pixida-aws-azure-login)


# @pixida/pixida-aws-azure-login
...
Install [Node.js](https://nodejs.org/) v12 or higher. Then install pixida-aws-azure-login with npm:

    npm install -g @pixida/pixida-aws-azure-login

You may need to install puppeteer dependency, if you're getting missing chrome or chromium message

    node <node_modules_dir>/pixida-aws-azure-login/node_modules/puppeteer/install.js
...
Install @pixida/pixida-aws-azure-login globally with npm:

    sudo npm install -g @pixida/pixida-aws-azure-login --unsafe-perm
...
Then install pixida-aws-azure-login:

    npm install -g @pixida/pixida-aws-azure-login

### Podman
> ⚠️ On Windows with WSL/Podman, your AWS config directory should be accessible at `C:\Users\<username>\.aws`  
> and mapped correctly. Use the `-v` option as follows:
>
> ```bash
> podman run -it -v /c/Users/<username>/.aws:/root/.aws:Z localhost/pixida-aws-azure-login -p default --no-sandbox
> ```


A Podman image has been built with Pixida-aws-azure-login preinstalled. You simply need to run the command with a volume mounted to your AWS configuration directory.

    podman run -it -v ~/.aws:/root/.aws:Z localhost/pixida-aws-azure-login -p default --no-sandbox


The Podman image is configured with an entrypoint so you can just feed any arguments in at the end.

You can also put the docker-launch.sh script into your bin directory for the pixida-aws-azure-login command to function as usual:

    sudo curl -o /usr/local/bin/pixida-aws-azure-login https://raw.githubusercontent.com/pixida-aws-azure-login/aws-azure-login/main/docker-launch.sh -L
    sudo chmod o+x /usr/local/bin/pixida-aws-azure-login

Now just run `@pixida/pixida-aws-azure-login`.

### Snap

### https://snapcraft.io/pixida-aws-azure-login
...
To configure the pixida-aws-azure-login client run:

    @pixida/pixida-aws-azure-login --configure
...
You'll need your [Azure Tenant ID and the App ID URI](#getting-your-tenant-id-and-app-id-uri). To configure a named profile, use the --profile flag.
    ...
    @pixida/pixida-aws-azure-login --configure --profile foo

##### GovCloud Support

To use pixida-aws-azure-login with AWS GovCloud, set the `region` profile property in your ~/.aws/config to the one of the GovCloud regions:
...
To use pixida-aws-azure-login with AWS China Cloud, set the `region` profile property in your ~/.aws/config to the China region:
...
As soon as you went through the full login procedure once, you can just use:
    ...
    @pixida/pixida-aws-azure-login --no-prompt

or

    @pixida/pixida-aws-azure-login --profile foo --no-prompt
...
Use the `HISTCONTROL` environment variable to avoid storing the password in your bash history (notice the space at the beginning):
    ...
    $ pixida-aws-azure-login

### Logging In

Once a@pixida/pixida-aws-azure-login is configured, you can log in. For the default profile, just run:
...
You will be prompted for your username and password. If MFA is required you'll also be prompted for a verification code or mobile device approval. To log in with a named profile:
    ...
    @pixida/pixida-aws-azure-login --profile foo
...
If you are logging in on an operating system with a GUI, you can log in using the actual Azure web form instead of the CLI:
    ...
    @pixida/pixida-aws-azure-login --mode gui
...
You can renew credentials for all configured profiles in one run. This is especially useful, if the maximum session length on AWS side is configured to a low value due to security constraints. Just run:
    ...
    @pixida/pixida-aws-azure-login --all-profiles

If you configure all profiles to stay logged in, you can easily skip the prompts:

    @pixida/pixida-aws-azure-login --all-profiles --no-prompt
